    ...
    <div className='col-span-full md:col-span-4 xl:col-span-5'>
      {
        projects
          ...
          .map((src, i) => {
            return ( 
              <img key={i} src={src} alt={`Live demo of project`} className={`rounded-lg ${(i === props.curSlide) ? null : 'hidden'} xl:h-full`} />
            );
          })
      }
    </div>
    ...
    <div className='col-span-full md:col-span-4 xl:col-span-3 h-[33rem] xxs:h-[28rem] xs:h-[24rem] sm:h-[18rem] md:h-[24rem] xl:h-auto'>
      {
        projects.map((project, i) => {
          return (
            <div key={i} className={`${(i === props.curSlide) ? null : 'hidden'} h-full flex flex-col justify-between items-center`}>
              {/* text */}
              <div>
...
const CarouselControls = (props) => {
  const getNextSlide = () => {
    return (props.curSlide === projects.length - 1) ? props.setCurSlide(0) : props.setCurSlide(props.curSlide + 1);
  }

  const getPrevSlide = () => {
    return (props.curSlide === 0) ? props.setCurSlide(projects.length - 1) : props.setCurSlide(props.curSlide - 1);
  }
